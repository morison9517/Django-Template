...
## 7. 本番(AWS)に出すときだけ必要なこと

**ハッカソン中は不要です。** 最後にデプロイする段階で読んでください。

1. `.env` の `DJANGO_DEBUG` を `False` にする(★必須。エラー画面に内部情報が出るため)
2. `DJANGO_SECRET_KEY` を長いランダム文字列に変える
3. `DJANGO_ALLOWED_HOSTS` と `DJANGO_CSRF_TRUSTED_ORIGINS` にサブドメインを追加する
4. CSS・JSを1か所に集める
...
> `DEBUG=False` にすると、Django自身はCSSを配らなくなります。
> 集めた `staticfiles/` をNginxから配る形になるためです。
> ここは切り替えを忘れると「本番だけデザインが崩れる」ので注意してください。
